...
func ParseEndpoint(
	...
	var (
		...
		sSAServerChangeGroupUserIDFlag = sSAServerChangeGroupFlags.String("user-id", "REQUIRED", "User ID")
		...
		sSAServerDeleteUserUserIDFlag = sSAServerDeleteUserFlags.String("user-id", "REQUIRED", "User ID")
		...
		sSAServerSaveDataGroupIDFlag = sSAServerSaveDataFlags.String("group-id", "REQUIRED", "Group ID")
		...
		sSAServerReturnDataListGroupIDFlag = sSAServerReturnDataListFlags.String("group-id", "REQUIRED", "Group ID")
		...
		sSAServerPickUpDataGroupIDFlag    = sSAServerPickUpDataFlags.String("group-id", "REQUIRED", "Group ID")
		sSAServerPickUpDataDataUserIDFlag = sSAServerPickUpDataFlags.String("data-user-id", "REQUIRED", "Data's User ID")
	)
...
func sSAServerChangeGroupUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] ssa-server change-group -body JSON -user-id INT
...
func sSAServerDeleteUserUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] ssa-server delete-user -body JSON -user-id INT
...
func sSAServerSaveDataUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] ssa-server save-data -body JSON -group-id STRING
...
func sSAServerReturnDataListUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] ssa-server return-data-list -body JSON -group-id STRING
...
Example:
    `+os.Args[0]+` ssa-server return-data-list --body '{
      "user_id": 537829
   }' --group-id "group-agieoa"
`, os.Args[0])
}

func sSAServerPickUpDataUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] ssa-server pick-up-data -body JSON -group-id STRING -data-user-id INT

データをサーバーから取得する
    -body JSON: 
    -group-id STRING: Group ID
    -data-user-id INT: Data's User ID
...
Example:
    `+os.Args[0]+` ssa-server pick-up-data --body '{
      ...
      "image_name": "Image_2017-05-25-26-32",
      "user_id": 65
   }' --group-id "group-isg" --data-user-id 5365
`, os.Args[0])
}
